...
E@${location.href}:`])) {
  ...
  var TryCachedEntities = async () => {
    ...
    Debug2("restoring cached entities data=", Data);
    await InsertEntities(Data.Entities);
    return true;
  };
  var PutCachedEntities = (Entity) => {
    if (Entity.length === 0) {
      return;
    }
  ...
  var Tinywave = async () => {
    const IsCachedEntitiesPassed = await TryCachedEntities().catch((Errors) => {
      Debug3("Failed to initialise cached entities", Errors);
      return false;
    });
